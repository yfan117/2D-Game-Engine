...
public class Browser extends JPanel {
	...
	static final File folders = new File(MapBuilder.dir + "\\test1");//Gets folder with tiles for game
	...
	public void createNodes() {
		for(int y = 0; y < maxSize.getHeight(); y += pixelH){
    		for(int x = 0; x < maxSize.getWidth(); x += pixelW){
    			if(y % 2 == 0) {
    				nodesB.get(0).add(new Nodes(x, y, true, 0));
    			}
    			else nodesB.get(0).add(new Nodes(x, y, false, 0));
